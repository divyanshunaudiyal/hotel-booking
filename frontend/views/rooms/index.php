...
<script src="<?= STATIC_URL; ?>js/datatablejs/datatable6.js?v=<?= STATIC_SITE_CONTENT_VERSION; ?>"></script>
...
<div class="row " style="margin-bottom: 4%;">  
    ...
    <div class="col-sm-3"  style="margin-top:4%;" >
        <a class="btn btn-block  btn-primary btn-rounded" href="<?= BASE_URL ?>rooms-create">Add rooms</a>
    </div>
</div>
                ...
                <table id="dynamic-table" class="table table-striped table-bordered display nowrap" style="width:100%;" >
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;"><?= \Yii::t('invest_quick', 'Price'); ?> </th>
                            <th style="text-align: center;"><?= \Yii::t('invest_quick', 'Total Rooms'); ?> </th>
                            <th style="text-align: center;"><?= \Yii::t('invest_quick', 'Is_active'); ?> </th>
                            ...
                            foreach ($data as $val) {
                                ...
                                <tr>
                                    ...
                                    <td style="text-transform:capitalize;">
                                        <?php if (!empty($val['room_type'])  ) { ?>

                                            <?php
                                            foreach ($roomtype as $type) {

                                                if ($val['room_type'] == $type['id']) {
                                                    echo $type['room_type']." - ".$type['room_category'];
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    <td>
                                        <?= !empty($val['price']) ? ucwords($val['price']) : ''; ?>
                                    </td>

                                    <td>
                                       
                                        <?= !empty($val['total_rooms']) ? ucwords($val['total_rooms']) : ''; ?>
                                    </td>
                                    ...
                                    <td style="text-align: center;">
                                        <a href="<?= BASE_URL ?>rooms/update?id= <?= $val['id'] ?>" title='Edit'><button type="button" class="btn btn-success">Edit</button></a>
                                        <a href="<?= BASE_URL ?>rooms/delete?id= <?= $val['rid'] ?>" title='Delete'><button type="button" class="btn btn-danger">Delete</button></a>
                                    </td>
